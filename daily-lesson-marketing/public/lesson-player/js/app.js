...
class KellyOS {
        ...
        if (this.state.currentLesson.age_expressions) {
            ...
            if (expression) {
                variant = {
                    ...
                    wisdomMoment: this.state.currentLesson.daily_fortune_elements?.universal_connection 
    ...
    renderPhase(variant) {
        ...
        } else if (phase === 'teaching' || phase === 'practice' || phase === 'question_1' || phase === 'question_2') {
            
            // V2 Logic
            if (this.state.currentLesson.core_lesson_structure) {
                const stepKey = (phase === 'teaching' || phase === 'question_1') ? 'question_1' : 'question_2';
                ...
                const v2Bucket = bucketMap[this.state.ageBucket] || 'young_adult';
                const examples = this.state.currentLesson.example_selector_data?.[stepKey + '_examples']?.[v2Bucket];
                
                if (structure && examples) {
                    mainText = examples.scenario; // In V2, scenario is the question/prompt
                    
                    // Option A
                    this.dom.choiceContainer.appendChild(this.createButton(examples.option_a, () => {
                         this.state.lessonPhase = (stepKey === 'question_1') ? 'practice' : 'wisdom';
                         this.reloadContentForAge();
                    }));
                    
                    // Option B
                    this.dom.choiceContainer.appendChild(this.createButton(examples.option_b, () => {
                         this.state.lessonPhase = (stepKey === 'question_1') ? 'practice' : 'wisdom';
                         this.reloadContentForAge();
                    }));
                }
