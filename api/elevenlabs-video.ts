...
async function generateLipSyncVideo(
  ...
  try {
    ...
    // Call ElevenLabs Image-to-Video endpoint
    ...
    if (contentType?.includes('video') || contentType?.includes('octet-stream')) {
      ...
      return {
        success: true,
        videoBuffer,
        durationMs: 0 // Unknown without parsing video
      };
    }
...
async function pollForVideoCompletion(
    ...
    try {
      ...
      for (const statusUrl of statusEndpoints) {
        ...
        if (result.status === 'failed' || result.status === 'error') {
          return { success: false, error: result.error || 'Generation failed', generationId };
        }
  ...
  return { success: false, error: 'Polling timeout', generationId };
}
