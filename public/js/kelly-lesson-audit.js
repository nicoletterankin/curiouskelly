...
(function() {
  ...
  const LessonInspector = {
    ...
    initSupabase() {
      ...
      // Try to create client from env vars
      if (typeof window.supabase !== 'undefined' && window.supabase.createClient) {
        const supabaseUrl = window.__SUPABASE_URL__ || 'https://tvjalxxsyryjphkforjv.supabase.co';
    ...
    async getFullAudit(dayNumber) {
      ...
      try {
        ...
        // 1. Load JSON lesson file (primary source of truth)
        await this.loadJsonLesson(audit, dayNumber);

        // 2. Query Supabase for ALL video assets
        ...
        // 4. Check local file system for generated assets
    ...
    /**
     * Load JSON lesson file
     */
    async loadJsonLesson(audit, dayNumber) {
      try {
        const jsonRes = await fetch(`/lessons/day-${dayNumber}.json`);
        if (jsonRes.ok) {
          audit.sources.json = `/lessons/day-${dayNumber}.json`;
          audit.assets.json = await jsonRes.json();
          audit.metadata = this.extractMetadata(audit.assets.json);
          audit.phases = this.extractPhases(audit.assets.json);
          
          // Extract audio paths from JSON
          audit.assets.audio.json = this.extractAudioPaths(audit.assets.json);
          audit.assets.audio.total += audit.assets.audio.json.length;
          
          // Extract language variants
          if (audit.assets.json.meta?.languages) {
            audit.assets.json.meta.languages.forEach(lang => audit.variants.languages.add(lang));
          }
          
          // Extract phase names
          if (audit.assets.json.phases) {
            Object.keys(audit.assets.json.phases).forEach(phase => audit.variants.phases.add(phase));
          }
        } else {
    ...
    async loadLocalAssets(audit, dayNumber, paddedDay) {
      const localPaths = [
        ...
        // Unified factory assets (Day 17 example structure)
    ...
    renderVideoSection(audit) {
      const videos = audit.assets.videos.supabase;
      if (videos.length === 0) {
        return '<div class="empty-state">No videos found in Supabase</div>';
      }

      // Group by phase
      const byPhase = {};
      videos.forEach(v => {
        if (!byPhase[v.phase]) byPhase[v.phase] = [];
        byPhase[v.phase].push(v);
      });

      return Object.entries(byPhase).map(([phase, phaseVideos]) => `
        <div class="phase-video-group">
          <h4>${phase.toUpperCase()} (${phaseVideos.length})</h4>
    ...
    async showInspector(dayNumber) {
      ...
      loadingPanel.innerHTML = `
        <div class="inspector-header">
          ...
          <button class="inspector-close" onclick="this.closest('.lesson-inspector-panel').remove()">×</button>
        </div>
        <div style="padding: 40px; text-align: center;">
          <div style="font-size: 18px; margin-bottom: 16px;">Gathering all assets...</div>
          <div style="font-size: 14px; color: rgba(255,255,255,0.6);">Querying Supabase, checking local files, loading JSON...</div>
        </div>
      `;
      ...
      try {
        ...
        loadingPanel.innerHTML = `
          ...
          <div style="padding: 40px; color: #ef4444;">Failed to load lesson: ${error.message}</div>
        `;
      }
    },
