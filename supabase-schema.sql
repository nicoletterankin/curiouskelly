...
-- ============================================
-- 12. LEARNING GROUPS TABLE
-- ============================================
-- For Share & Connect feature: group learning with friends & family
...
-- Policies - members can view their groups
...
-- ============================================
-- 14. DAILY LESSON STATS TABLE
-- ============================================
-- Tracks aggregate stats per day for Global Perspectives
...
CREATE TABLE IF NOT EXISTS public.daily_lesson_stats (
  ...
  countries_list TEXT[], -- Array of country codes
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TRIGGER update_daily_stats_updated_at BEFORE UPDATE ON public.daily_lesson_stats
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_learning_groups_updated_at BEFORE UPDATE ON public.learning_groups
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- ============================================
-- 15. FUNCTION: Update daily stats on lesson completion
-- ============================================
...
BEGIN
  ...
  IF NEW.completed AND (OLD.completed IS NULL OR NOT OLD.completed) THEN
    ...
    DO UPDATE SET 
      learners_count = public.daily_lesson_stats.learners_count + 1,
      lessons_completed = public.daily_lesson_stats.lessons_completed + 1,
      updated_at = NOW();
  END IF;
