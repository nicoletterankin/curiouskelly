...
class CalendarApp {
    constructor() {
        ...
        // Progress and Streak State
        this.progress = {
            streak: 0,
            lastVisit: null,
            completedLessons: [], // Array of lesson IDs (or days if IDs missing)
            settings: {
                age: '6-12',
                language: 'en'
            }
    ...
    async init() {
        ...
        this.currentAge = this.progress.settings.age;
        this.currentLanguage = this.progress.settings.language;
    ...
    loadProgress() {
        try {
            ...
            if (saved) {
                ...
                // Check streak validity (reset if missed a day)
                this.validateStreak();
            }
        } catch (e) {
    ...
    saveProgress() {
        try {
            this.progress.settings.age = this.currentAge;
            this.progress.settings.language = this.currentLanguage;
            localStorage.setItem('kelly_progress', JSON.stringify(this.progress));
        } catch (e) {
    ...
    validateStreak() {
        if (!this.progress.lastVisit) return;
        
        const last = new Date(this.progress.lastVisit);
        const now = new Date();
        ...
        // (1 day diff means yesterday, so streak is kept)
        if (diffDays > 1) {
            this.progress.streak = 0;
        }
        
        // If visited today, don't increment yet (wait for lesson completion)
        // logic handled in completeLesson
    }

    completeLesson(lessonDay) {
        // Add to completed list if not already there
        if (!this.progress.completedLessons.includes(lessonDay)) {
            this.progress.completedLessons.push(lessonDay);
            ...
            let last = null;
            if (this.progress.lastVisit) {
                last = new Date(this.progress.lastVisit);
                last.setHours(0, 0, 0, 0);
            }
            
            // Increment streak if this is the first lesson completed TODAY
            // OR if it's the first lesson ever
            if (!last || today > last) {
                this.progress.streak++;
                this.progress.lastVisit = now.toISOString();
                
                // Show streak celebration?
                this.animateStreakCounter();
            }
            
            this.saveProgress();
            this.updateStreakUI();
            
            // Re-render views to show checkmarks
            this.renderView(); 
        }
    }

    updateStreakUI() {
        const counter = document.getElementById('streak-count');
        if (counter) {
            counter.textContent = this.progress.streak;
        }
    }
    ...
    updateSettingsUI() {
        ...
        if (ageSelector) ageSelector.value = this.currentAge;
    ...
    setupEventListeners() {
        ...
        document.getElementById('age-selector').addEventListener('change', (e) => {
            ...
            this.saveProgress(); // Save setting change
    ...
    renderToday() {
        ...
        const isCompleted = this.progress.completedLessons.includes(lesson.day);
        ...
        card.innerHTML = `
            ...
            ${isCompleted ? 
                `<div style="margin-top: 12px; color: #34c759; font-weight: 600; font-size: 13px;">✓ Completed</div>` : ''
            }
    ...
    renderMonthView() {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if (lesson && this.progress.completedLessons.includes(lesson.day)) classes.push('completed');
    ...
    renderWeekView() {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            if (lesson) {
                ...
                html += `
                    <div class="week-day-card ${lesson.has_dna ? 'has-dna' : ''} ${isCompleted ? 'completed' : ''}" 
    ...
    renderPhase() {
        ...
        if (this.currentPhase === 'welcome') {
            ...
            // Mark lesson as complete when reaching wisdom
            this.completeLesson(this.currentLesson.day);
            ...
            content.innerHTML = `
                <div class="wisdom-content">
                    ...
                    <div style="text-align: center; margin-top: 20px;">
                        ...
                        <div style="color: #34c759; font-weight: 600; margin-top: 8px;">Lesson Complete!</div>
                    </div>
                </div>
            `;
        } else {
